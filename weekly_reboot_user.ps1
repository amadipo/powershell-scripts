New-Item -path "c:\temp" -Name "Scripts" -ItemType "Directory" -Force
$file = "c:\temp\Scripts\shot.ps1"
    New-Item -path "c:\temp\Scripts\" -Name "shot.txt" -ItemType "File" -Force
    ...
    Add-Content -Path "c:\temp\Scripts\shot.txt" -Value "$j `r`n$k `r`n$h `r`n$l `r`n$p `r`n$o `r`n$z `r`n$d `r`n$n `r`n$a `r`n$w `r`n$r `r`n$b `r`n$aa `r`n$ab `r`n$ac "
    Move-Item -Path "c:\temp\Scripts\shot.txt" -Destination "c:\temp\Scripts\shot.ps1" -Force

$file = '"c:\temp\Scripts\shot.ps1"'
$A = New-ScheduledTaskAction -Execute "%SystemRoot%\system32\WindowsPowerShell\v1.0\powershell.exe" -Argument "-windowstyle hidden -NoProfile -NoLogo -NonInteractive -ExecutionPolicy Bypass -File $file "
...
Register-ScheduledTask Weekly_Shutdown1 -InputObject $D -Force
